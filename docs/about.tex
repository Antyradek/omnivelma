...
\begin{figure}[H]
\centering
 \includegraphics[width=\textwidth]{graphics/mecanum_dirs.pdf}
...
Na tym momencie program może je wyświetlić, ale nie wygeneruje na ich podstawie sterowania.
...
W ramach przygotowań do pracy udało się zainstalować, skonfigurować i przetestować wszystkie moduły środowiska.
Została zdobyta wiedza na temat możliwych problemów z instalacją i używaniem ich w przyszłości.
Jakie systemy operacyjne jak dobrze współpracują z systemem ROS i na czym nie da się go zainstalować.
Testowanie odbyło się za pomocą budowy własnego, abstrakcyjnego modelu i zastosowania do niego zmodyfikowanych programów przykładowych.

Udało się sprawdzić, jakie są możliwości stworzenia struktury projektu, aby nie umieszczać modeli i kodu osobno, aby zachować porządek.
Ustawiono wszelkie systemy kontroli wersji użyte także do budowy tego dokumentu.

Aby zabrać się do pracy, należy przejrzeć sporą ilość materiałów na temat budowy kół i modelowania ich wirtualnie, a także powodów nieścisłości obliczeń.
Dobrze jest także obejrzeć dalsze dokumenty zawarte w bibliografiach materiałów.

Należy eksperymentować z różnymi rozwiązaniami ewentualnych uproszczeń modelu, sprawdzić, jak rozwiązywali ten problem inni.
Przetestować, czy wspomaganie symulacji za pomocą karty graficznej nie rozwiąże problemu nadmiernych obliczeń.
